...
class UsageTrackingAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("User", {"fields": ("user", "is_premium_display")}),
        (
        ...
        ("Storage", {"fields": ("total_storage_bytes", "storage_used_display")}),
        (
            "Lifetime Statistics",
            {
                "fields": (
        ...
        (
            "Usage Summary",
            {"fields": ("usage_summary_display",), "classes": ("collapse",)},
        ),
    )
    ...
    def storage_used_display(self, obj):
        ...
        color = (
            "#10b981"
            if percentage < 70
            else "#f59e0b" if percentage < 90 else "#ef4444"
        )
        return format_html(
...
class OrganizationAdmin(admin.ModelAdmin):
    ...
    def seats_display(self, obj):
        ...
        color = (
            "#10b981"
            if percentage < 80
            else "#f59e0b" if percentage < 100 else "#ef4444"
        )
        return format_html(
            '<span style="color: {};">{} / {}</span>', color, obj.seats_used, obj.seats
        )
...
class OrganizationMembershipAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Membership", {"fields": ("user", "organization", "role", "is_active")}),
        ("Invitation", {"fields": ("invited_by", "invited_at", "accepted_at")}),
        (
            "Deactivation",
