...
from .models import JourneyMilestone, Deadline
...
def dashboard(request):
    ...
    total_checklists = (
        user.exam_checklists.count() if hasattr(user, "exam_checklists") else 0
    )
    ...
    if hasattr(user, "exam_checklists"):
        upcoming_exams = user.exam_checklists.filter(
            exam_date__gte=date.today(), exam_completed=False
...
def journey_dashboard(request):
    ...
    milestones = JourneyMilestone.objects.filter(user=request.user).order_by("-date")[
        :5
    ]
...
def journey_timeline_partial(request):
    ...
    return render(
        request,
        "core/partials/journey_timeline.html",
        {
            "timeline": timeline,
        },
    )
...
def claim_progress(request):
    ...
    user_data = (
        ...
        .annotate(
            doc_count=Count(
                "documents", filter=Q(documents__is_deleted=False), distinct=True
            ),
    ...
    recent_documents = Document.objects.filter(user=user, is_deleted=False).order_by(
        "-created_at"
    )[:5]

    # Most recent evidence gap analysis for recommendations
    latest_evidence_gap = (
        EvidenceGapAnalysis.objects.filter(user=user).order_by("-created_at").first()
    )
    evidence_readiness_score = (
        latest_evidence_gap.readiness_score if latest_evidence_gap else None
    )
    evidence_gaps = []
    ...
    score_components.append(
        {
            "name": "Evidence Gaps Analyzed",
            "score": evidence_score,
            "max": 25,
            "complete": evidence_analyses >= 1
            and evidence_readiness_score
            and evidence_readiness_score >= 70,
        }
    )
    ...
    score_components.append(
        {
            "name": "Personal Statement Drafted",
...
def toggle_deadline(request, pk):
    ...
    return render(
        request,
        "core/partials/deadline_item.html",
        {
            "deadline": deadline,
        },
    )
...
def submit_feedback(request):
    ...
    return render(
        request,
        "core/partials/feedback_thanks.html",
        {
            "feedback": feedback,
        },
    )
...
def feedback_form(request):
    ...
    return render(
        request,
        "core/partials/feedback_form.html",
        {
...
def admin_stats_dashboard(request):
    ...
    docs_week = Document.objects.filter(
        created_at__gte=week_ago, is_deleted=False
    ).count()
    docs_month = Document.objects.filter(
        created_at__gte=month_ago, is_deleted=False
    ).count()
    ...
    try:
        ...
        ai_stats = {
            ...
            "interactions_today": AgentInteraction.objects.filter(
                created_at__date=today
            ).count(),
            "interactions_week": AgentInteraction.objects.filter(
