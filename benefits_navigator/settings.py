...
MIDDLEWARE = [
    "core.middleware.HealthCheckMiddleware",  # Health check - must be before CommonMiddleware for DO health checks
...
CELERY_BROKER_CONNECTION_RETRY_ON_STARTUP = (
    True  # Retry broker connection on startup (Celery 6.0+ default)
)
...
CELERY_BEAT_SCHEDULE = {
    ...
    "enforce-pilot-data-retention": {
        ...
        "schedule": crontab(
            hour=3, minute=30
        ),  # 3:30 AM daily (after cleanup-old-health-metrics)
    },
...
FEATURES = {
    ...
    # Shared Features
    "doc_search": env.bool(
        "FEATURE_DOC_SEARCH", default=True
